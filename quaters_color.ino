...
#define NUMOFCOLORS  30  
// the color jump of each iteration
#define COLOR_JUMP  4

// delay between iterations
#define DELAY 10000 // delay between colors change (msec)
#define FADE 5 // delay fade (msec)
...
void loop() {
  ...
  int c4 = 150;
  while (1) {
    light_trace(c1,c2,c3,c4,1); // fade_int    
    delay (DELAY);
    light_trace(c1,c2,c3,c4,0); // fade_out    
...
void light_trace(int c1, int c2, int c3, int c4,int fade_in) {
  float power = 0;
  for (int i = 0 ; i< 10; ++i) {
    if (fade_in) {
      power = i/10.0;
...
//    Serial.print(power);
    ...
    light_quarter (c1,0,q1_end,power);
    light_quarter (c2,q1_end+1,q2_end,power);
    light_quarter (c3,q2_end+1,q3_end,power);
    light_quarter (c4,q3_end+1,NUMPIXELS-1,power);
    pixels.show(); // This sends the updated pixel color to the hardware.    
    delay (FADE);
  }
}

void light_quarter (int color, int start_led, int end_led,float power) {        
